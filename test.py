...
def main(args):
    ...
    if rank == 0:  # 在第一个进程中打印信息，并实例化tensorboard
        ...
        tb_writer = SummaryWriter(log_dir=r'/root/data/hcm')
        if os.path.exists("/root/data/hcm/weights") is False:
            os.makedirs("/root/data/hcm/weights")

    # 实例化训练数据集
    train_data = MyDataset(r'/root/data/hcm/bl_data_poc_oversampling_train.csv')
    test_data = MyDataset(r'/root/data/hcm/bl_data_poc_oversampling_test.csv')
    ...
    for epoch in range(args.epochs):
        ...
        if rank == 0:
            ...
            torch.save(model.module.state_dict(), "/root/data/hcm/weights/model-{}.pth".format(epoch))
